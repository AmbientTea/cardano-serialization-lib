...
impl num_traits::SaturatingSub for Value {
    fn saturating_sub(&self, v: &Self) -> Self {
        let multiasset = self
            .multiasset()
            .unwrap_or_default()
            .saturating_sub(&v.multiasset().unwrap_or_default());
        Self {
            coin: self.coin.saturating_sub(&v.coin),
            multiasset: (!multiasset.is_zero()).then_some(multiasset),
        }
    }
}
...
impl num_traits::SaturatingAdd for Value {
    fn saturating_add(&self, v: &Self) -> Self {
        let multiasset = self
            .multiasset()
            .unwrap_or_default()
            .saturating_add(&v.multiasset().unwrap_or_default());
        Self {
            coin: self.coin.saturating_add(&v.coin),
            multiasset: (!multiasset.is_zero()).then_some(multiasset),
        }
    }
}
